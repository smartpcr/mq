...
public class RecoveryService
{
    ...
    public async Task<RecoveryResult> RecoverAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var snapshot = await _persister.LoadSnapshotAsync(cancellationToken);
            ...
            if (snapshot != null)
            {
                ...
                {
                    await _buffer.EnqueueAsync(message, cancellationToken);
                ...
                {
                    await _deduplicationIndex.TryAddAsync(kvp.Key, kvp.Value, cancellationToken);
            ...
            var journalOperations = await _persister.ReplayJournalAsync(
                result.LastVersion,
                cancellationToken);
    ...
    private async Task ApplyOperationAsync(OperationRecord operation, CancellationToken cancellationToken)
    {
        switch (operation.OperationCode)
        {
            ...
            case OperationCode.Acknowledge:
                ...
                await _buffer.AcknowledgeAsync(operation.MessageId, cancellationToken);
                break;
            ...
            case OperationCode.Requeue:
                ...
                await _buffer.RequeueAsync(operation.MessageId, cancellationToken);
                break;
            ...
            case OperationCode.DeadLetter:
                ...
                await _buffer.RemoveAsync(operation.MessageId, cancellationToken);
                break;
    ...
    {
        var messages = await _buffer.GetAllMessagesAsync(cancellationToken);
        int recovered = 0;
        ...
        foreach (var message in messages)
        {
            ...
            {
                // Requeue the message
                await _buffer.RequeueAsync(message.MessageId, cancellationToken);
                recovered++;
            }
        }
